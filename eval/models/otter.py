...
class EvalModel(BaseEvalModel):
    ...
    def get_caption_prompt(self, prompt="What does the image describe?", caption=None) -> str:
        return (
            f"<image>User: {prompt} GPT:<answer>{caption if caption is not None else ''}{'<|endofchunk|>' if caption is not None else ''}"
        )
